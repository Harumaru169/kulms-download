...
from .components import Resource, Site
...
class AbstractApiClient(ABC):
    ...
    @abstractmethod
    async def fetch_partial_site_list_without_resource(
        self, page_size: int, offset: int
    ) -> list[Site]:
        pass
    ...
    @abstractmethod
    def get_stream(self, url: str) -> AbstractAsyncContextManager[httpx.Response]:
        pass
...
class ApiClient(AbstractApiClient):
    def __init__(
        self, cookie_fetcher: AbstractCookieFetcher, constants: Constants
    ) -> None:
        super().__init__()
    ...
    async def fetch_partial_site_list_without_resource(
        ...
        try:
            response = await self.http_client.get(
                self._site_list_json_url(page_size, offset)
            )
    ...
    async def fetch_resource_list(self, site: Site) -> list[Resource]:
        try:
            ...
            url_prefix = (
                f"https://lms.gakusei.kyoto-u.ac.jp/access/content/group/{site.id}/"
            )
            container_prefix = f"/content/group/{site.id}/"
            return [
                Resource.from_dict(item, site, url_prefix, container_prefix)
            ...
            raise NetworkError(
                f"リソース一覧のJSON解析に失敗しました: {site.title}"
            ) from e
