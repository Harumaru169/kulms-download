...
def main():
    ...
    try:
        ...
        kulms_download_cli.console.print("予期しないエラーが発生しました")
...
class KulmsDownloadCli:
    def __init__(self) -> None:
        ...
        self.remote_cookie_fetcher = RemoteCookieFetcher(
            self.credential_manager, self.password_app_opener, self.constants
        )
        self.local_cookie_fetcher = LocalCookieManager(self.constants)
        self.cookie_fetcher = CookieFetcher(
            self.remote_cookie_fetcher, self.local_cookie_fetcher, self.constants
        )
    ...
    async def main(self):
        ...
        cli_func = await questionary.select(
            "コマンドを選択", choices=choices
        ).unsafe_ask_async()
        await cli_func()
        ...
        async with self.api_client as client:
            ...
            dest_dir = Path(
                await questionary.path(
                    ...
                    validate=lambda path_str: (
                        True
                        if Path(path_str).is_dir()
                        else "存在するディレクトリを指定してください"
                    ),
                ).unsafe_ask_async()
            )
            ...
            with self.console.status("ダウンロード中..."):
                await self.resource_downloader.download_resources_for_site_list(
                    selected_site_list, dest_dir
                )
            print("ダウンロードが完了しました")
    ...
    async def settings_cli(self):
        choices = [
            ...
            Choice(
                "自動入力するECS-IDとパスワードの保存/削除", self.password_setting_cli
            ),
            Choice(
                "認証画面でパスワード管理アプリを自動で開く", self.open_app_setting_cli
            ),
            Choice(
                "ワンタイムパスワードの設定/削除", self.one_time_password_setting_cli
            ),
        ]

        next_func = await questionary.select(
            "=== 設定 ===", choices=choices
        ).unsafe_ask_async()
        await next_func()
    ...
    async def delete_cookie_cli(self):
        print(
            "Cookieデータはログイン後、1時間程度保持されています。削除すると、次回使用時にログインウインドウが立ち上がります。"
        )
        confirmation = await questionary.confirm(
            "Cookieデータを削除しますか？"
        ).unsafe_ask_async()
    ...
    async def password_setting_cli(self):
        num = await questionary.select(
            ...
            choices=[Choice("更新,追加", 0), Choice("削除", 1)],
        ).unsafe_ask_async()

        if num == 0:
            username = await questionary.password(
                "ESC-IDのユーザー名を入力:"
            ).unsafe_ask_async()
    ...
    async def one_time_password_setting_cli(self):
        num = await questionary.select(
            "=== ワンタイムパスワードの設定 ===",
            choices=[Choice("設定/更新", 0), Choice("削除", 1)],
        ).unsafe_ask_async()

        if num == 0:
            uri = await questionary.password(
                "ワンタイムパスワードの設定URIを入力 (例: otpauth://totp/......)"
            ).unsafe_ask_async()
    ...
    async def open_app_setting_cli(self):
        print(
            "KULMSのログインウインドウが起動した時に、設定することで同時にパスワード管理アプリを起動させることができます。ワンタイムパスワードの入力がスムーズにできて便利です。"
        )
