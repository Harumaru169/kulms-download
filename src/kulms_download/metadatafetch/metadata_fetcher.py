...
class MetadataFetcher(AbstractMetadataFetcher):
    ...
    async def fetch_site_list(self) -> list[Site]:
        ...
        while True:
            partial_site_list = (
                await self.client.fetch_partial_site_list_without_resource(
                    page_size=self.constants.metadata_fetch_page_size, offset=offset
                )
            )
    ...
    def _build_resource_tree(cls, site: Site, resource_list: list[Resource]):
        ...
        def recursive(target_content: Resource):
            if not resource_list:
                return  # 空っぽなら打ち切る
            children = cls._pop_all(
                lambda c: target_content.shouldHaveAsChild(c), resource_list
            )
